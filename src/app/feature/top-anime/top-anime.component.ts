...
export class TopAnimeComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    if (page) await this.getTopAnimeByPage(page);
    else await this.getTopAnimeList();
  }
    ...
    this.animeService.getTopAnimes().subscribe({
      next: (data: any) => {
        for (let item of data.data) {
          ...
          let jpg_url: AnimeImageUrl = new AnimeImageUrl(
            image.jpg.image_url as string,
            image.jpg.large_image_url as string,
            image.jpg.small_image_url as string
          );
          let webp_url: AnimeImageUrl = new AnimeImageUrl(
            image.webp.image_url as string,
            image.webp.large_image_url as string,
            image.webp.small_image_url as string
          );
          ...
          for (let genre_item of genres) {
            animeGenres.push(
              new AnimeGenre(
                genre_item.mal_id as number,
                genre_item.name as string,
                genre_item.type as string,
                genre_item.url as string
              )
            );
          }
          this.topAnimeList.push(
            new AnimeInfo(
              item.mal_id as number,
